...
# --- Helper functions (custom_collate_fn, etc.) remain unchanged ---
...
def run_experiment(csv_path, npz_path, val_ratio, random_seed, experiment_name):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n▶️ Starting Experiment: {experiment_name} on {device}")
...
if __name__ == "__main__":
    ...
    ALL_SEEDS = [
        # 你已经跑过的 3 个种子:
        46, 124, 42,
        # 7 个新的种子 (你可以修改这些新种子的数字):
        101, 202, 303, 404, 505, 606, 707
    ]
    # 定义已经完成的种子
    COMPLETED_SEEDS = [46, 124, 42]

    print(f"开始实验，总共 {len(ALL_SEEDS)} 个种子。")
    print(f"已完成的种子 (将加载结果): {COMPLETED_SEEDS}")
    ...
    for seed in ALL_SEEDS:
        ...
        if seed in COMPLETED_SEEDS:
            # --- 这是一个已完成的种子, 加载结果 ---
            print(f"\n{'=' * 20} 加载已完成种子的结果: {seed} {'=' * 20}")
            history_csv_path = os.path.join('results', experiment_name, 'training_history.csv')

            try:
                if not os.path.exists(history_csv_path):
                    print(f"⚠️ 警告: 找不到已完成种子 {seed} 的历史文件: {history_csv_path}")
                    print("     请确保 'results' 文件夹和对应的实验子文件夹存在。")
                    print("     跳过此种子。")
                    continue

                history_df = pd.read_csv(history_csv_path)
                if history_df.empty:
                    print(f"⚠️ 警告: 历史文件 {history_csv_path} 为空。跳过。")
                    continue

                # 根据 'val_mae' 找到最佳的 epoch
                best_epoch_stats = history_df.sort_values(by='val_mae').iloc[0]

                results = {
                    'mae': best_epoch_stats['val_mae'],
                    'rmse': best_epoch_stats['val_rmse'],
                    'r2': best_epoch_stats['val_r2']
                }

                print(f"✅ 已加载结果: MAE={results['mae']:.4f}, RMSE={results['rmse']:.4f}, R2={results['r2']:.4f}")
                all_results.append(results)

            except Exception as e:
                print(f"❌ 加载种子 {seed} 的结果时出错: {e}。跳过。")
    ...
    # --- 打印总结到控制台 ---
    print(f"\n\n{'=' * 50}")
    print(f"📊 聚合结果 (总共 {len(all_results)} 个种子) 📊")
    print(f"{'=' * 50}")
    print("\n--- 每个种子的原始结果 ---")
    ...
    # --- [MODIFIED] 格式化并保存结果到 TXT 文件 ---
    output_txt_path = 'training_results_summary_10SEED.txt'
    ...
    try:
        with open(output_txt_path, 'w', encoding='utf-8') as f:
            f.write(f"聚合 {len(all_results)} 个种子的结果:\n")  # 添加了上下文
            f.write(f"{r2_str}\n")
            f.write(f"{mae_str}\n")
            f.write(f"{rmse_str}\n")

        print(f"\n✅ 聚合结果摘要已保存到: {output_txt_path}")
