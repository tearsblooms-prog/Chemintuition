...
def normalize_reactant_pair(r1: str, r2: str) -> tuple:
    ...
    # Since we are enforcing pairs, both s1 and s2 should ideally be present,
    # but this logic remains safe for sorting purposes.
...
def generate_candidates_from_pools(original_data_csv: str,
    ...
    # 3. Create Reactant Combinations (Two reactants ONLY)
    # MODIFIED: Removed the `+ [(r, '') for r in top_reactants]` part
    reactant_combos = list(itertools.combinations(top_reactants, 2))
    print(f"Generated {len(reactant_combos)} reactant combinations (R1+R2 only).")
