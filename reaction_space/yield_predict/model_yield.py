# model_cankao_contrastive.py
import torch
import torch.nn as nn
from torch_geometric.nn import Set2Set, SAGEConv  # <-- 改动：导入 SAGEConv
from torch_geometric.data import Batch


class DualChannelMPNN(nn.Module):
    """
    MPNN module modified with a Dual-Channel Attention mechanism.
    - Two separate gating channels are introduced: 'structure' and 'reactivity'.
    - The 'reactivity' channel's gates are returned to compute an auxiliary sparsity loss.
    - An interface is provided to return node-level features and gates for visualization.
    """
    ...
    def __init__(self, node_in_feats, edge_in_feats, hidden_feats=64,
        ...
        # --- 【改动】将 GINEConv 替换为 SAGEConv ---
        # SAGEConv 不需要一个额外的 `nn` 或 `edge_dim` 参数
...
class YieldNet(nn.Module):
    ...
    - It maintains the contrastive learning objective.
    - It provides a comprehensive output for visualization.
    """
    ...
    def forward(self, r1_graph_batch, r2_graph_batch, p_graph_batch,
        ...
        if return_node_features:
            ...
            # (Note: condition chemicals are omitted in visualization mode for simplicity)

            final_concat_feats = torch.cat([unified_reactant_feats, p_graph_feats], dim=1)
            out = self.predict(final_concat_feats)

            # Return predictions and detailed visualization data for each main molecule
