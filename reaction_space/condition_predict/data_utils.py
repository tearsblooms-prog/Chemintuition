...
class ReactionConditionTokenizer:
    ...
    def _discretize_temp(self, t):
        ...
        # --- 【最终修正】---
        # 将所有超出范围的值，归类到最接近的、已存在的词元中
        elif t > 300:
            return 'TEMP_291_301'  # 归入最高温区间
        elif t < -100:
            return 'TEMP_-100_-95'  # 将所有低于-100的温度归入最低温区间
        elif -100 <= t < -20:  # 这是之前版本缺失的、处理-100到-20的正确逻辑
            bin_start = -100 + math.floor((t - (-100)) / 5) * 5
            return f'TEMP_{bin_start}_{bin_start + 5}'
        else:  # 兜底处理
            return 'TEMP_undefined'

    def _discretize_duration(self, d):
        # <--- 【最终修正】使用与 range() 行为一致的 < 进行判断，确保逻辑无懈可击 ---
        if d == -1:
    ...
    def _undiscretize_temp(self, token):
        if token == 'TEMP_undefined': return -999.0
    ...
    def fingerprint_to_tokens(self, fingerprint_640d):
        ...
        for i in range(CONDITION_NUM_STEPS):
            ...
            for seg_idx in range(4):
                ...
                # --- 【修改 1/2】---
                # 不再合并温度信息，而是分别为起始温度和结束温度生成独立的Token
                # 之前的逻辑: temp_to_process = t_end if t_end != -999 else t_start
                # 之前的代码: step_tokens.append(self.vocab[self._discretize_temp(temp_to_process)])
    ...
    def tokens_to_fingerprint(self, token_ids):
        ...
        for i, step_toks in enumerate(step_sequences):
            ...
            for seg_idx in range(4):
                ...
                # --- 【修改 2/2】---
                # 现在需要获取4个Token来重建一个时间段：起始温度, 结束温度, 时长, 回流
                # 之前的代码: temp_tok, dur_tok, reflux_tok = get_next_token(), get_next_token(), get_next_token()
                ...
                if t_start_tok and t_end_tok and dur_tok and reflux_tok:
                    # 将解析出的Token值填回指纹向量的正确位置
                    # 之前的代码硬编码了起始温度为-999: fingerprint[i, fp_seg_start_idx] = -999.0
                    ...
                    # 如果Token不足，则用默认值填充
                    fingerprint[i, fp_seg_start_idx:fp_seg_start_idx + 4] = torch.tensor([-999.0, -999.0, -1.0, 0.0])
                # --- 【修改结束】---
