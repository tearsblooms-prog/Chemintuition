...
def get_atom_features(atom: Chem.Atom) -> np.ndarray:
    ...
    is_potential_center = 1.0 if (atomic_num not in [1, 6] or atom.GetFormalCharge() != 0 or atom.GetNumRadicalElectrons() != 0 or atom.GetIsAromatic()) else 0.0
...
def main():
    source_csv_path = 'data/demo_reaction_with_fingerprint.csv'
    id_to_chem_json_path = 'data/demo_id_to_chemical.json'
    cleaned_csv_path = 'data/demo_reaction_cleaned.csv'
    graphs_npz_path = 'data/demo_data_graphs.npz'

    global ATOM_FEATURE_SIZE
    ATOM_FEATURE_SIZE = 47
    print(f"Starting preprocessing demo data... (Atom Feature Size: {ATOM_FEATURE_SIZE})")

    graphs_to_save = {}

    # --- 1. Process demo_id_to_chemical.json ---
    ...
    # --- 2. Process demo main dataset CSV file ---
    ...
    # Check for 'products_smiles' column and rename to 'product_smiles' if needed
    if 'products_smiles' in raw_df.columns and 'product_smiles' not in raw_df.columns:
        print("Detected 'products_smiles' column, renaming to 'product_smiles' for compatibility...")
        raw_df.rename(columns={'products_smiles': 'product_smiles'}, inplace=True)
    elif 'product_smiles' not in raw_df.columns:
        print(f"Error: neither 'product_smiles' nor 'products_smiles' found in '{source_csv_path}'.")
        return

    def check_row_data(row):
        # Check reactant1
        if not is_valid_smiles(row['reactant1_smiles']):
            return False

        # Check reactant2 (if exists)
        r2_s = row.get('reactant2_smiles')
        if pd.notna(r2_s) and str(r2_s).strip() and not is_valid_smiles(str(r2_s)):
            return False

        # Check product
        p_s = str(row['product_smiles'])
        if pd.isna(p_s) or not any(is_valid_smiles(s) for s in p_s.split(';') if s.strip()):
            return False

        # Check condition_fingerprint existence
        if 'condition_fingerprint' not in row or pd.isna(row['condition_fingerprint']):
            return False
    ...
    # Add dummy y_val for inference compatibility
    if 'y_val' not in cleaned_df.columns:
        print("Adding dummy 'y_val' column for inference compatibility...")
        cleaned_df['y_val'] = 0.0

    print(f"Data cleaning complete. Valid data: {len(cleaned_df)} rows. Saving to: {cleaned_csv_path}")
    ...
    print("demo data preprocessing complete!")
